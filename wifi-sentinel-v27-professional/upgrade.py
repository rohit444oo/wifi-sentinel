...
import re, textwrap
p=Path('/mnt/data/v31/app.py')
s=p.read_text()
start=s.index('def assess(rows):')
end=s.index('\ndef save_results(results):', start)
new=r'''def _history_for(c, bssid, limit=30):
...
def assess(rows):
    ...
    baseline={clean_bssid(r["bssid"]):dict(r) for r in c.execute("SELECT * FROM baseline")}
    histories={}
    for raw in rows or []:
        b=clean_bssid(raw.get("bssid"))
        if re.fullmatch(r"[0-9A-F]{2}(?::[0-9A-F]{2}){5}", b): histories[b]=_history_for(c,b,30)
    c.close()
    results=[]

    for raw in rows or []:
        r=dict(raw); b=clean_bssid(r.get("bssid"))
        if not re.fullmatch(r"[0-9A-F]{2}(?::[0-9A-F]{2}){5}", b): continue
        ssid=(r.get("ssid") or "<hidden>").strip() or "<hidden>"
        sec=normalize_security(r.get("security")); prev=baseline.get(b); approved_ap=approved.get(b)
        ...
        if approved_ap:
            status="Authorized"
            expected=normalize_security(approved_ap.get("security"))
            if expected != sec:
        ...
        if prev:
            ...
            old,newsig=signal_int(prev.get("signal")),signal_int(r.get("signal"))
        ...
        prior_signals=[signal_int(x.get("signal")) for x in prior]
        median_signal=_median(prior_signals)
        current_signal=signal_int(r.get("signal"))
        ...
        prior_sec=[normalize_security(x.get("security")) for x in prior]
        ...
        if prev:
            ...
            if normalize_security(prev.get("security"))!=sec: fp_match-=20
            if (prev.get("vendor") or "")!=vendor_for(b): fp_match-=12
        ...
        indicators={
            ...
            "security_downgrade": bool(prev and normalize_security(prev.get("security"))!=sec),
        ...
        results.append({
            **r,"ssid":ssid,"bssid":b,"security":sec,"vendor":vendor_for(b),"status":status,"risk":risk,"level":level,
...
'''
s=s[:start]+new+s[end:]
# add helpers and routes before /history
marker='@app.get("/history")\ndef history():'
insert=r'''@app.get("/access-points")
def access_points():
    """Display every AP from the latest completed assessment, with no UI truncation."""
    c=db(); scan=c.execute("SELECT * FROM scans ORDER BY id DESC LIMIT 1").fetchone()
    rows=[dict(x) for x in c.execute("SELECT * FROM observations WHERE scan_id=? ORDER BY risk DESC, ssid COLLATE NOCASE, bssid",(scan["id"],))] if scan else []
    c.close(); dashboard_enrich(rows)
    for x in rows:
        x["confidence"]=96 if x.get("status")=="Authorized" else min(98,35+int(x.get("evidence_count",0))*9)
    return render_template("access_points.html", rows=rows, scan=dict(scan) if scan else None)
...
# remove old duplicate access_points block if exists
s=s.replace('''@app.get("/access-points")\ndef access_points():\n    """Display every AP observed in the latest completed assessment."""\n    c = db()\n    scan = c.execute("SELECT * FROM scans ORDER BY id DESC LIMIT 1").fetchone()\n    rows = []\n    if scan:\n        rows = [dict(x) for x in c.execute(\n            "SELECT * FROM observations WHERE scan_id=? ORDER BY risk DESC, ssid COLLATE NOCASE, bssid",\n            (scan["id"],)\n        )]\n    c.close()\n    for x in rows:\n        x["security"] = normalize_security(x.get("security"))\n        x["trust_score"] = trust_score(x)\n        x["fingerprint_score"] = int(x.get("fingerprint_score") or 0)\n        x["anomaly_score"] = int(x.get("anomaly_score") or 0)\n        x["policy_label"] = SECURITY_POLICY.get(x["security"], SECURITY_POLICY["UNKNOWN"])["label"]\n        # Stored observations predate confidence support; derive a conservative value.\n        evidence = sum(bool(v) for v in [\n            x.get("status") in ("Suspicious", "Possible Rogue", "Configuration Mismatch"),\n            bool(x.get("changed")),\n            x.get("security") in ("OPEN", "WEP", "WPA"),\n            int(x.get("fingerprint_risk", 0) or 0) >= 25,\n            int(x.get("behavior_risk", 0) or 0) >= 20,\n        ])\n        x["confidence"] = 96 if x.get("status") == "Authorized" else min(98, 35 + evidence * 12)\n        if x.get("status") == "Unverified": x["confidence"] = min(x["confidence"], 55)\n    return render_template("access_points.html", rows=rows, scan=dict(scan) if scan else None, logged=logged_in(), user=session.get("user"), user_role=role())\n\n\n''','')
p.write_text(s)
