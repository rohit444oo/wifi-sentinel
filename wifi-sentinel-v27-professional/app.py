...
app = Flask(__name__)
app.secret_key = os.environ.get("WIFI_SENTINEL_SECRET", "change-this-local-secret")
...
def scan_windows():
    ...
    for i, header in enumerate(headers):
        ...
        # Guard against malformed/localized netsh output where metadata is
        # captured as the SSID value.
        ssid = header.group(1).strip() or "<hidden>"
        if re.match(
            r"^(Network type|Authentication|Encryption|BSSID|Signal|Radio type|Channel|Basic rates|Other rates)\s*:",
            ssid, re.I
        ):
            ssid = "<hidden>"
...
def assess(rows):
    """Evidence-based wireless assessment using authorization and historical fingerprints."""
    c = db()
    ...
    for raw in rows or []:
        ...
        if approved_ap:
            ...
            if normalize_security(approved_ap.get("security")) != sec:
                identity_risk += 8
                security_risk += 15
                changed.append("security differs from authorized inventory")
                reasons.append(f"Security differs from authorized inventory ({approved_ap.get('security')} → {sec})")
                status = "Configuration Mismatch"
        else:
            identity_risk += 18
            reasons.append("BSSID is not in the authorized inventory")
            if same_ssid:
                identity_risk += 28
                fingerprint_risk += 22
                reasons.append("Authorized SSID observed from a different BSSID")
                status = "Suspicious"
            else:
                status = "Unverified"
        ...
        if sec == "OPEN":
            security_risk += 30
            reasons.append("No Wi-Fi encryption detected")
        elif sec == "WEP":
            security_risk += 28
            reasons.append("Legacy WEP encryption detected")
        elif sec == "WPA":
            security_risk += 12
            reasons.append("Legacy WPA security detected")

        if prev:
            if prev.get("channel") != r.get("channel"):
                changed.append("channel changed"); baseline_risk += 8
            if normalize_security(prev.get("security")) != sec:
                changed.append("security changed"); baseline_risk += 12
            oldsig, newsig = signal_int(prev.get("signal")), signal_int(r.get("signal"))
            if oldsig is not None and newsig is not None:
                delta = abs(newsig - oldsig)
                if delta >= 35:
                    changed.append("large signal change"); baseline_risk += 7
                elif delta >= 20:
                    changed.append("signal changed"); baseline_risk += 3
            if (prev.get("band") or "") != (r.get("band") or "") and (prev.get("band") or r.get("band")):
                changed.append("band changed"); baseline_risk += 6
            if (prev.get("radio") or "") != (r.get("radio") or "") and (prev.get("radio") or r.get("radio")):
                changed.append("radio type changed"); baseline_risk += 5
            if changed:
                reasons.append("Historical change: " + ", ".join(changed))
        else:
            reasons.append("First observation for this AP")
            baseline_risk += 2

        behavior_risk = min(35, baseline_risk)
        if prev:
            if len(changed) >= 3:
                behavior_risk += 10
            elif len(changed) == 2:
        ...
        if prev:
            if prev.get("channel") != r.get("channel"): fingerprint_match -= 18
            if normalize_security(prev.get("security")) != sec: fingerprint_match -= 20
            if (prev.get("vendor") or "") != vendor_for(b): fingerprint_match -= 10
            if (prev.get("band") or "") != (r.get("band") or "") and (prev.get("band") or r.get("band")): fingerprint_match -= 8
            if (prev.get("radio") or "") != (r.get("radio") or "") and (prev.get("radio") or r.get("radio")): fingerprint_match -= 6
        fingerprint_match=max(0,min(100,fingerprint_match))
        fingerprint_risk=max(fingerprint_risk,100-fingerprint_match)

        risk=round(min(100,
            0.30*min(100,identity_risk)+
            0.24*min(100,security_risk)+
            0.18*min(100,baseline_risk)+
            0.14*min(100,behavior_risk)+
            0.14*min(100,fingerprint_risk)
        ))
        if not approved_ap:
            risk=max(risk,18 if not same_ssid else 42)
        ...
        if status not in ("Authorized","Configuration Mismatch"):
            if indicators>=3: status="Possible Rogue"
            elif same_ssid and not approved_ap: status="Suspicious"
            else: status="Unverified"

        risk=min(100,max(0,int(risk)))
        level="Critical" if risk>=85 else "High" if risk>=65 else "Medium" if risk>=40 else "Low"
        reason="; ".join(reasons) if reasons else "Matches authorized inventory and historical baseline"

        results.append({
            **r,"ssid":ssid,"bssid":b,"security":sec,"vendor":vendor_for(b),
            "status":status,"risk":risk,"level":level,"reason":reason,
            "changed":", ".join(changed),
            "anomaly":min(100,behavior_risk*2),
            "anomaly_score":min(100,behavior_risk*2),"ml_anomaly":0,
